#include <stdio.h>
#include <stdlib.h>
#include <sys/stat.h>
#include <sys/types.h>
#include <fcntl.h>
#include <unistd.h>
#include <stdbool.h>
#include <string.h>
#include <getopt.h>
#include <openssl/sha.h>
#include "blowfish.h"
#define DEF_MODE S_IRUSR|S_IWUSR
bool dflag = false; //Decrypt/Encrypt flag
bool Fflag =false;

void sha1_hash(const char *input, unsigned char *output) {
    SHA_CTX sha_ctx;
    SHA1_Init(&sha_ctx);
    SHA1_Update(&sha_ctx, input, strlen(input));
    SHA1_Final(output, &sha_ctx);
}

void print_help(char *command)
{
	printf("secret encripta o desincripta un archivo usando el algoritmo Blowfish.\n");
	printf("uso:\n %s [-d] -k <key> <nombre_archivo|nombre_carpeta>\n", command);
	printf(" %s -h\n", command);
	printf("Opciones:\n");
	printf(" -h\t\t\tAyuda, muestra este mensaje\n");
	printf(" -d\t\t\tDesincripta el archivo en lugar de encriptarlo.\n");
	printf(" -f\t\t\tEncripta/Desencripta una carpeta en vez de un archivo\n");
	printf(" -k <key>\t\tEspecífica la clave (key) de encriptación, 8 bytes en hex.\n");
}

int main(int argc, char **argv)
{
	struct stat mi_stat;
	char *input_file = NULL;
	char *key_arg_str = NULL;

	int opt, index;
	
	while ((opt = getopt (argc, argv, "dhk:")) != -1){
		switch(opt)
		{
			case 'd':
				dflag = true;
				break;
			case 'h':
				print_help(argv[0]);
				return 0;
			case 'k':
				key_arg_str = optarg;
				break;
			case 'f':
				Fflag = true;
			case '?':
			default:
				fprintf(stderr, "uso: %s [-d] -k <key> <nombre_archivo>\n", argv[0]);
				fprintf(stderr, "     %s -h\n", argv[0]);
				return 1;
		}
	}

	/* Aquí recoge argumentos que no son opción, por ejemplo el nombre del input file */
	for (index = optind; index < argc; index++)
		input_file = argv[index];

	if(!input_file){
		fprintf(stderr, "Especifique el nombre del archivo.\n");
		fprintf(stderr, "uso: %s [-d] -k <key> <nombre_archivo>\n", argv[0]);
		fprintf(stderr, "     %s -h\n", argv[0]);
		return 1;
	}else{
		/* Ejemplo como verificar existencia y tamaño de un archivo */
		if(stat(input_file, &mi_stat) < 0){
			fprintf(stderr, "Archivo %s no existe!\n", input_file);
			return 1;
		}else
			printf("Leyendo el archivo %s (%ld bytes)...\n", input_file, mi_stat.st_size);
	}

	/* Valida la clave de encriptación */
/*	if(key_arg_str){
		if(strlen(key_arg_str) != 16){
			fprintf(stderr, "Error en tamaño de la clave de encriptación.\n");
			return 1;
		}
	}else{
		fprintf(stderr, "Error al especificar la clave de encriptación.\n");
		fprintf(stderr, "uso: %s [-d] -k <key> <nombre_archivo>\n", argv[0]);
		fprintf(stderr, "     %s -h\n", argv[0]);
		return 1;
	}

*/	
	
	/*TO-DO: Encriptar o desencriptar archivo input_file usando Blowfish */
	/*TIP: Revisar https://github.com/B-Con/crypto-algorithms/blob/master/blowfish_test.c */
	
	
	/* RETO1 - Extraer valor númerico de la clave en ASCII hex. (ver readme) */
	// key_arg/for/sscanf()
	BYTE key_arg[8];
	unsigned char *output = (char*) calloc(SHA_DIGEST_LENGTH, sizeof(char));
	
	unsigned short byte, i;
	for(int i=0; i<8; i++){

		sscanf(key_arg_str +2*i, "%2hx",&byte);
		key_arg[i]= (BYTE) byte;
	}

	// Tarea
	sha1_hash(key_arg_str,output);
	for(int j =0; j<8; j++){
		printf("%02x", output[j]);
	}
	printf("\n");
	//sha1_hash(key_arg_str,output);
	
	/* RETO2 - generar nombre nuevo de archivo con .enc o .dec. (validar con variable dflag. Usar calloc, strcpy, strcat) */
	 int byteEOF, byteDot;
	byteEOF = 1;
	byteDot = byteEOF;
	char *output_file = (char*) calloc(strlen(input_file)+2,sizeof(char)); //strlen()
	strcpy(output_file,input_file);
	if(dflag){
		strcat(output_file,".dec");
	}else{
		strcat(output_file,".enc");
	}


	///* RETO3 - encriptar o desencriptar (ver blowfish_test.c) *///
	
	/* crear archivo (tipo: O_CREAT, O_TRUNC, y O_WRONLY,   y con modo: S_IRUSR y S_IWUSR): */
	 int fd_output = open(output_file, O_CREAT|O_TRUNC|O_WRONLY,S_IRUSR|S_IWUSR);

	/* configuracion del blowfish_key: */
	// input_buf[...] = {0}   //p1
	// ouput_buf[...]         //enc_buf
	// Blowfish key
	BYTE output_buf[8];
	BYTE input_buf[8] = {0};
	// b_k_setup(...)
	BLOWFISH_KEY key;
	blowfish_key_setup(key_arg,&key,8);
	/* leer archivo  */
	// int fd_input = open() //readonly
	int fd_input =open(input_file,O_RDONLY,0);
	/* recorrer archivo, encriptar o desencriptar, y guardar: */
	
	// while read(...)
	while(read(fd_input,input_buf,8)>0){
		if(dflag){
			blowfish_decrypt(input_buf, output_buf,&key);
		}else{	
			blowfish_encrypt(input_buf,output_buf,&key);
		}
		write(fd_output,output_buf,8);
		memset(input_buf, 0, 8);
	}

	if(dflag){
		printf("desencriptacion exitosa");
	}else{
		printf("Encriptacion exitosa");
	}
	close(fd_output);
	close(fd_input);
	free(output_file);
	//// if / else
	////// blowfish_decrypt(...);
	////// blowfish_encrypt(...);
		
	//// write();
	//// memset(input_buf, 0, sizeof input_buf);
	
	// if dflag printf mensaje exito encriptado/desencriptado
	
	
	// closes files and free calloc
	
}
